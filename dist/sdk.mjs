...
const TRAIT_SUMMARY_KEY$1 = "pgs:trait-summary"; // needed in getScoresPerTrait
...
async function loadScoreStats() {
	...
	try {
		...
		if (cached?.summary && isCacheWithinMonths$1(cached.savedAt, 3)) {
			...
			if (output) {
				output.textContent = `Loaded ${formatNumber$1(cached.summary.totalScores)} cached scores summary + trait-linked score cache (${cached.savedAt}).`;
			}
			return results;
		}
		const summaryForPlot = {
		...
		if (output) {
			output.textContent = `Loaded ${formatNumber$1(summary.totalScores)} scores from PGS Catalog and built trait-linked score cache.`;
		}
...
const BASE$1 = "https://www.pgscatalog.org/rest";
...
async function fetchAllTraits({ pageSize = 50, maxPages = Infinity } = {}) {
  ...
  while (page < maxPages) {
    const url = `${BASE$1}/trait/all?format=json&limit=${pageSize}&offset=${offset}`;
...
export { fetchScores, fetchTraits, getTxts, loadAllScores, loadScoreStats, loadScores, localforage };
